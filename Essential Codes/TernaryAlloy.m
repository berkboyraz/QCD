...
x=0.52;
InGaAs52 = x*InAs + (1-x)*GaAs - x*(1-x)*InGaAs_bowing;   
x=0.37;
...
x=0.66;
InGaAs66 = x*InAs + (1-x)*GaAs - x*(1-x)*InGaAs_bowing; 
...
x=0.69;
AlInAs69 = x*AlAs + (1-x)*InAs - x*(1-x)*AlInAs_bowing; 

x=0.48;
AlInAs = x*AlAs + (1-x)*InAs - x*(1-x)*AlInAs_bowing;   % Al0.48In0.52As is lattice matched on InP
x=0.47;
AlInAs47 = x*AlAs + (1-x)*InAs - x*(1-x)*AlInAs_bowing;   
x=0.80;
...
x=0.16;
AlAsSb16 = x*AlAs + (1-x)*AlSb - x*(1-x)*AlAsSb_bowing;  
...
AlGaAs_bowing(idx_Eg6c) = -0.127 + 1.310*x;   % from Vurgaftman
AlGaAs_bowing(idx_VBO)  =  0.27  -  1.05*x;   % to adjust VBO in order to fit CBO from THz QCL
AlGaAs35 = x*AlAs + (1-x)*GaAs - x*(1-x)*AlGaAs_bowing;
